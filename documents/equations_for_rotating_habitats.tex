...
\begin{document}
	...
	\begin{abstract}
		...
		A way to calculate the mass of a rotating habitat is also provided, with notes about why 10~tonnes per square metre is the default for radiation shielding reasons.
	...
	\begin{itemize}
		\item $a_c$ is the acceleration caused by the change in height
		...
		\item $\omega$ is the angular velocity of the habitat
	...
	At the same time, since your environment is rotating, if you jump and stay in the air, your direction of movement will appear to be rotating relative to the environment.
	If you manage to jump and stay in the air so long that the habitat rotated by one quarter turn, all of your vertical velocity will appear to have been converted into horizontal velocity.
	This too could be explained as a force or an acceleration acting on you:
	
	$$ a_r = \frac{v_v}{t/4} $$
	
	\begin{itemize}
		\item $a_r$ is the apparent acceleration caused by rotation
	...
	$$ \sigma_\theta = R_c \cdot \rho_{wall} \cdot (g + \frac{P}{\rho_a})$$
	
	$$ R_c = \frac{\sigma_\theta}{s \cdot \rho_{wall} \cdot (g + \frac{P}{\rho_a})}$$
	
	$$ s = \frac{\sigma_\theta}{R_c \cdot \rho_{wall} \cdot (g + \frac{P}{\rho_a})}$$
	
	where:
	
	\begin{itemize}
		\item $s$ is the safety factor (failure stress divided by maximum acceptable stress)
		
		\item $\sigma_\theta$ is the hoop stress in the habitat
		
		\item $R_c$ is the radius of the cylinder
		
		\item $P$ is the internal atmospheric pressure
		...
		\item $g$ is the centrifugal acceleration provided by the habitat's rotation
	\end{itemize}
	
	Safety factor must always be larger than 1, if the safety factor is exactly 1 then the habitat is operating exactly at its theoretical breaking point, where the centrifugal force acting on the mass of its walls combined with the atmospheric pressure it holds is exactly enough to overpower the tensile strength of the walls and rupture the habitat.
	...
	As an example habitat I will present one with the following characteristics:
	
	\begin{itemize}
		\item 1gee internal gravity ($g = 9.807m/s^2$)
		
		\item 1 atmosphere internal pressure ($P = 101325 Pa$)
		
		\item Made from annealed 304 stainless steel (tensile strength 210MPa and density 7900$kg/m^3$)
		
		\item 10 tonnes per square meter of hull (standard for, $\rho_a = 10 000kg/m^2$)
		
		\item safety factor 2 (meaning that stress in the habitat is naturally 2 times less than what it would take to break it)
	\end{itemize}
	
	$$ R_c = \frac{210MPa}{2 \cdot 7900kg/m^3 \cdot (9.807m/s^2 + \frac{101325 Pa}{10 000kg/m^2})} = 666.585m$$
	
	As such this example habitat is a little over 666 meters in radius.
	...
	For clarity this equation deducing stress from the habitat's rotation and composition will be written as such:
	
	$$ \sigma_{\theta r} \approx R_c \cdot g \cdot \rho_{wall} $$
	
	where:
	
	\begin{itemize}
		\item $\sigma_{\theta r}$ is the hoop stress in the habitat resulting from its own rotation
		
		\item $R_c$ is the radius of the habitat
		
		\item $g$ is the centrifugal acceleration provided by the habitat's rotation
		
		\item $\rho_{wall}$ is the density of the material from which the habitat is made
	\end{itemize}
	
	This could notably already be the final form if the habitat in question has no atmosphere.
	
	\noindent \rule{\linewidth}{0.5pt}
	
	\begin{center}
		\textbf{accounting for internal atmospheric pressure}
	\end{center}
	
	As previously mentioned the hoop stress equation is as such:
	
	$$ \sigma_\theta \approx \frac{R_c \cdot P}{t_d} $$
	
	To make use of it for calculating stress caused in the habitat by atmospheric pressure, thickness of the wall must be known. The thickness of the wall can be calculated simply as such:
	
	$$ t_d = \frac{\rho_a}{\rho_{wall}} $$
	...
	$$ \sigma_{\theta p} \approx \frac{R_c \cdot P \cdot \rho_{wall}}{\rho_a} $$
	
	As such for the final hoop stress in the habitat the two causes of stress should be combined, ie. summing up the stress caused by internal atmospheric pressure with the stress caused by rotation:
	
	$$ \sigma_\theta = \sigma_{\theta r} + \sigma_{\theta p} = R_c \cdot g \cdot \rho_{wall} + \frac{R_c \cdot P \cdot \rho_{wall}}{\rho_a} = R_c \cdot \rho_{wall} \cdot (g + \frac{P}{\rho_a})$$
	
	where:
	
	\begin{itemize}
		\item $\sigma_{\theta p}$ is the hoop stress resulting from internal atmospheric pressure
