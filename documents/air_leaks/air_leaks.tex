...
\begin{document}
	...
	$$ \lambda = \frac{V \cdot P_{start}}{A \cdot \sqrt{T}} \cdot 0.000000002004645164\frac{s \cdot \sqrt{K}}{m \cdot Pa} $$
...
def mass_flow_equation(A,P,T):
    mass_flow = A * P * math.sqrt(T) * 0.04041469727
    return mass_flow
...
print(f"lost half of air pressure in {time_passed} seconds")
    ...
    \begin{itemize}
        \item A room of volume=$1000m^3$ at 1 atmosphere with a hole of area=$0.0001m^2$ takes $\approx$118.634 seconds to drop down to 50\% of the starting pressure
        
        \item That same room takes $\approx$237.266 seconds to drop down to 25\% of the starting pressure, which is twice as long - this seems to confirm my assumption that the pressure loss will be well approximated by a halving time
        
        \item If the volume of the room is increased by 10 times, time to lose half pressure also grows by a factor of 10.
        
        \item If the area of the hole is decreased by a factor of 10, time to lose pressure grows by a factor of 10.
        
        \item If the temperature is lowered by a factor of 4, the time to lose pressure grows by a factor of 2.
    \end{itemize}
